...
void Rat::move()
{
    if (m_idleTurnsRemaining > 0)
    {
        ...
        m_arena->history().record(m_row, m_col); //this indicates the rat is poisoned, thus, it records a turn at the position of the rat
        return;
    }
    ...
    if (m_health < INITIAL_RAT_HEALTH)
    {
        ...
        if (!(isDead()))
        {
            m_arena->history().record(m_row, m_col); //this also indicates the rat is poisoned, but not dead. Record to history that rat has spent a turn at this position.
        }
    }
